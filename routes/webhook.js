...
const commands = { // If too long, Can't print...
     "!시간규칙": {
        func: () => `시간은 다음과 같은 규칙으로 입력할 수 있습니다.|>>([월~일](-[월~일]):[0~23](-[0~23]), )*\n예시: 월-14 or 월-수:2 or 월:12-19 or 월-수:2-23 or 금-화:23-2, 수:3-4, 목:1, 금:2, 토-일:1-23|콜론 사이에 공백이 있으면 안되며, 24시를 사용해야 합니다. 시간이 24시를 넘긴 경우 다음날 새벽으로 인식합니다.|`,
        intro: '시간 규칙을 확인합니다. 처음 쓰시는 분들은 꼭 확인하세요!|'
    }, 
    "!테스트": {
        func: (args, userid) => `${userid}님의 새로운 메시지: ${args}|testtest`,
    ...
    "!사용법": {
        func: async () => Object.keys(commands).map(x => x + ": " + commands[x].intro).join(""),
        intro: "사용법을 호출합니다."
    }
}
